...
const state = {
    ...
    socketCombat_legend: socketIo(process.env.VUE_APP_SOCKET_URL+'/combat_legend', { auth: {}, autoConnect: false, reconnection: true, reconnectionDelay: 5000, transports: ['websocket']}),
    ...
    socketSlots: socketIo(process.env.VUE_APP_SOCKET_URL+'/slots', { auth: {}, autoConnect: false, reconnection: true, reconnectionDelay: 5000, transports: ['websocket']}),
...
const actions = {
    ...
    socketListenGeneral({ getters, dispatch }) {
        ...
        getters.socketGeneral.on('settings', (data) => { dispatch('generalSocketSettings', data); console.log(data) });
    ...
    socketConnectCombat_legend({ getters, dispatch }) {
        if(getters.socketCombat_legend.connected === true) { getters.socketCombat_legend.removeAllListeners(); }
        if(getters.authToken !== null) { getters.socketCombat_legend.auth.token = getters.authToken; }

        getters.socketCombat_legend.disconnect().connect();

        dispatch('socketListenCombat_legend');
    },
    socketDisconnectCombat_legend({ getters }) {
        getters.socketCombat_legend.removeAllListeners();
        getters.socketCombat_legend.disconnect();
    },
    socketListenCombat_legend({ getters, dispatch }) {
        getters.socketCombat_legend.on('init', (data) => { dispatch('combat_legendSocketInit', data); });
        getters.socketCombat_legend.on('game', (data) => { dispatch('combat_legendSocketGame', data); });
    },
    ...
    socketConnectSlots({ getters, dispatch }) {
        if(getters.socketSlots.connected === true) { getters.socketSlots.removeAllListeners(); }
        if(getters.authToken !== null) { getters.socketSlots.auth.token = getters.authToken; }

        getters.socketSlots.disconnect().connect();

        dispatch('socketListenSlots');
    },
    socketDisconnectSlots({ getters }) {
        getters.socketSlots.removeAllListeners();
        getters.socketSlots.disconnect();
    },
    socketListenSlots({ getters, dispatch }) {
        getters.socketSlots.on('init', (data) => { dispatch('slotsSocketInit', data); });
    },
