<template>
    <div class="combat_legend-game-header">
        <div class="header-info">
            ...
            GAME ID = {{combat_legendGame._id}}
        </div>
        <div class="header-actions">
            <div class="actions-sound">
                <div v-on:click="modalToggleSound()" class="sound-toggle" v-bind:class="{ 'toggle-active': soundCombat_legend === 1 }"></div>
                SOUND ON
            </div>
...
<script>
    ...
    export default {
        name: 'Combat_legendGameHeader',
        props: ['combat_legendGame'],
        methods: {
            ...mapActions([
                'notificationShow', 
                'soundSetCombat_legend'
            ]),
            modalToggleSound() {
                if(this.soundCombat_legend === 1) { this.soundSetCombat_legend(0);
                } else { this.soundSetCombat_legend(1); }
            },
            modalCopyButton() {
                const el = document.createElement('textarea');
                el.value = 'BLOXFlame.com/CombatLegend?game=' + this.combat_legendGame._id;
...
<style scoped>
    ...
    .combat_legend-game-header .header-info {
        font-family: 'Rubik';
        font-size: 16px;
        font-weight: 600;
        color: #c1c1c1;
    ...
    .combat_legend-game-header .info-title {
        display: flex;
        align-items: center;
        margin-bottom: 8px;
        font-size: 28px;
    ...
    .combat_legend-game-header .info-title svg {
        margin-right: 12px;
        fill: #ffffff;
    }

    .combat_legend-game-header .header-actions {
        display: flex;
        align-items: center;
    }

    .combat_legend-game-header .actions-sound {
        display: flex;
        align-items: center;
        margin-right: 20px;
        font-size: 12px;
    ...
    .combat_legend-game-header .sound-toggle {
        width: 45px;
        height: 15px;
        position: relative;
        margin-right: 12px;
    ...
    .combat_legend-game-header .sound-toggle::before {
        content: '';
        width: 100%;
        height: 100%;
        position: absolute;
    ...
    .combat_legend-game-header .sound-toggle::after {
        content: '';
        width: 25px;
        height: 19px;
        position: absolute;
    ...
    .combat_legend-game-header .sound-toggle.toggle-active::after {
        transform: translate(20px, 0);
        background: linear-gradient(255deg, #f00 0%, #f00 100%);
    }

    .combat_legend-game-header .header-actions button.button-copy {
        display: flex;
        align-items: center;
        font-size: 12px;
        font-weight: 700;
    ...
    .combat_legend-game-header .header-actions button.button-copy:hover {
        color: #ffffff;
    }

    .combat_legend-game-header .header-actions button.button-copy svg {
        margin-right: 12px;
        fill: #5e768e;
        transition: fill 0.3s ease;
    }

    .combat_legend-game-header .header-actions button.button-copy:hover svg {
        fill: #ffffff;
    }
