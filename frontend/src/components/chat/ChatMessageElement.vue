    ...
    export default {
        ...
        computed: {
            ...
            chatGetGameLink() {
                ...
                if(
                    this.message.message.replace(/&#x2F;/g, '/').match(/(BLOXFlame\.com\/battles\/[0-9a-fA-F]{24})/) !== null ||
                    this.message.message.replace(/&#x2F;/g, '/').match(/(BLOXFlame\.com\/duels\?game=[0-9a-fA-F]{24})$/) !== null
                ) {
                    link = this.message.message.replace(/&#x2F;/g, '/').replace(/.*BLOXFlame\.com/, '');
                }
...
<style scoped>
    ...
    .chat-message-element.element-rain {
        ...
        background: linear-gradient(100deg, #454444 5%, #45444430%, #454444 50%, rgba(0, 0, 0, 0) 80%), rgba(3, 16, 27, 0.32);
    ...
    .chat-message-element .element-message.message-admin button.button-user span.user-username {
        background: linear-gradient(250deg, #f00 0%, #f00 100%), #a6cae8;
        -webkit-background-clip: text;
        -webkit-text-fill-color: transparent;
        background-clip: text;
        text-fill-color: transparent;
