<template>
    <div class="login-roblox">
        ...
        <div class="roblox-info">
            Warning! Please use your alt account only. <button v-on:click="loginTermsButton()" class="button-terms">Terms of Service</button>.
        </div>
        <div class="roblox-buttons">
            <button v-on:click="$parent.modalSetTab(tab === 'roblox cookie' ? 'roblox cookie' : 'roblox cookie')" class="button-toggle" v-bind:class="{ 'button-active': tab === 'roblox cookie' }" v-bind:disabled="authSendLoginLoading === true || authStep !== 'login'">
...
<style scoped>
    ...
    .login-roblox button.button-toggle.button-active .button-inner {
        ...
        background: radial-gradient(60% 60% at 50% 50%, rgba(255, 0, 0, 0.2) 0%, rgba(0, 0, 0, 0) 100%), 
                    linear-gradient(255deg, rgba(255, 0, 0, 0.05) 0%, rgba(0, 170, 109, 0.05) 100%);
    }

    .login-roblox button.button-action .button-inner {
        color: #ffffff;
        background: linear-gradient(250deg, #f00 0%, #f00 100%);
    }
