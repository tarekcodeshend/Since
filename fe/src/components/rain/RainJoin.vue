        ...
        <div class="join-inner">
            <div class="inner-type">
                <div v-if="generalRain.active.type === 'user'" class="type-user">
                    ...
                    <div class="user-text">
                        <div v-html="generalRain.active.creator.username" class="text-username"></div>HOSTED A<span
                            class="text-green">RAIN</span>
                    </div>
                </div>
                <div v-else class="type-site">
                    <div class="site-text">
                        <span class="text-green">IT’S RAINING!</span>
                    </div>
                </div>
            </div>
            ...
            <button v-on:click="rainClaimButton()" class="button-claim" v-bind:disabled="rainIsClaimed === true">
                <div class="button-inner">
                    <span>{{ rainIsClaimed === true ? 'JOINED' : 'JOIN RAIN' }}</span>
                </div>
            </button>
        </div>
    </div>
...
.rain-join .info-amount {
    ...
    box-shadow: 0px 0px 8.804px 0px rgba(0, 0, 0, 0.20) inset, 0px 5.283px 20.309px 0px rgba(0, 0, 0, 0.12);
}
...
.rain-join .amount-value {
    color: #FFF;
    text-overflow: ellipsis;
    text-shadow: 0px 2.352px 2.352px rgba(0, 0, 0, 0.23);
    font-family: Poppins;
    font-size: 9.391px;
    font-style: normal;
    font-weight: 600;
...
.inner-info .rain-remain {
    width: 45%;
    border-radius: 7px;
    border: 1.5px dashed #D4AF3C;
    background: linear-gradient(180deg, rgba(29, 24, 38, 0.00) 0%, rgba(29, 24, 38, 0.10) 100%), #241D2E;
    display: flex;
    justify-content: space-between;
    align-items: center;
    color: white;
...
.rain-join button.button-claim .button-inner {
    ...
    align-items: center;
    background: #D4AF3C;
    box-shadow: 0px 2px 0px 0px #886d1e;
    border-radius: 7px;
    transition: all .2s;
}

.rain-join button.button-claim .button-inner:hover {
    background: #dbba56;
}
